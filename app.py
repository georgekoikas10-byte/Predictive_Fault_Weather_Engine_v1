...
st.set_page_config(
    page_title="Predictive Fault Weather Engine v4 FIXED",
    page_icon="⛈️",
    layout="wide",
...
TTLP_LIST = [
    ...
    "Τ.Τ.Λ.Π. ΑΙΤΩΛΟΑΚΑΡΝΑΝΙΑΣ",
    "Τ.Τ.Λ.Π. ΑΛΥΣΙΔΑΣ",
    "Τ.Τ.Λ.Π. ΑΜΑΡΟΥΣΙΟΥ",
    "Τ.Τ.Λ.Π. ΑΜΠΕΛΟΚΗΠΩΝ",
    "Τ.Τ.Λ.Π. ΑΝΑΤΟΛΙΚΗΣ ΑΤΤΙΚΗΣ",
    "Τ.Τ.Λ.Π. ΑΡΓΟΛΙΔΑΣ",
    "Τ.Τ.Λ.Π. ΑΡΕΩΣ",
    "Τ.Τ.Λ.Π. ΑΡΚΑΔΙΑΣ",
    "Τ.Τ.Λ.Π. ΑΡΤΑΣ",
...
st.markdown('<p class="main-title">⛈️ Predictive Fault Weather Engine v4 FIXED</p>', unsafe_allow_html=True)
st.markdown('<p class="subtitle">Ιστορικό 1 έτους / Μόνο βλάβες / Ιστορικές κακοκαιρίες / Περιοχές-Τεχνικά Τμήματα / Πρόγνωση 7 ημερών</p>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    selected_ttlp = st.selectbox("ΤΤΛΠ στόχευσης", TTLP_LIST, index=0)
...
try:
    ...
    one_year_back = end_d - timedelta(days=365)
    scoped = scoped[scoped["date"].dt.date >= one_year_back].copy()
    ...
    with tab4:
        st.subheader("🎯 ΤΤΛΠ Target")
        st.markdown(f"""
        <div class="card">
            <div class="big">Επιλεγμένο ΤΤΛΠ: {selected_ttlp}</div>
            <p>Σε αυτή την έκδοση κρατάμε όλα τα προηγούμενα dashboards και προσθέτουμε στόχευση ΤΤΛΠ.</p>
            <p><b>Σημαντικό:</b> Το workbook έχει slicer με ΤΤΛΠ, όμως το Streamlit δεν μπορεί ακόμη να εφαρμόσει το Excel slicer/pivot filter απευθείας. Για πραγματική εισροή ανά ΤΤΛΠ πρέπει να διαβάσουμε το pivot cache ή να έχουμε raw sheet.</p>
            <p>Μέχρι να κουμπώσει το extraction, το prediction παραμένει συνολικό/weighted μόνο για βλάβες, και η επιλογή ΤΤΛΠ λειτουργεί ως target προβολής.</p>
        </div>
        """, unsafe_allow_html=True)
        st.subheader("Τρέχουσα συνολική πρόβλεψη για σύγκριση")
        st.dataframe(prediction, use_container_width=True)
        st.subheader("Περιοχές/τεχνικά τμήματα που πιέζονται")
        if area_risk.empty:
            st.warning("Δεν βρέθηκε area stock table στο workbook.")
        else:
            st.dataframe(area_risk, use_container_width=True)
